...
function fetchJSON(){
var RestAPIurl = "https://script.google.com/macros/s/AKfycbxXf4a4dqiT9mAZz4JJCT-soTeHFjowWwWeY9nrEeukLMvgq7FA/exec";
fetch(RestAPIurl)
    .then(d => d.json())
    .then(d => {
        return d[0].data;
...
bot.on("text",(ctx => {
 var id = ctx.chat.id; 
    ctx.telegram.sendMessage(id, answer + "\n" + "Please click on the buttons below." , {
        reply_markup: {
            inline_keyboard: [
                [{text: "Enter Place", switch_inline_query_current_chat: ""}],
                [{text: "Choose Floor", callback_data: "Floor list"}]
            ]
        }
    });  
